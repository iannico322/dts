...
const Header = ({ navActive, setActive }:any) => {
  ...
  return (
    ...
    <div className=" animate__animated animate__slideInDow bg-blue3 flex justify-between items-center w-full  py-3 border-b-[0px] border-accent  ">
        ...
        <nav className=" lg:hidden mr-10  text-accent-foreground flex gap-20 uppercase items-center">
          <Link className=" flex items-center  gap-2 hover:font-semibold text-xs transition-all duration-75 text-white" to="" >
            <img className=" h-5 object-contain" alt="ProfileIcon" src={ProfileIcon}/>
          Account
          </Link>
          <Link className=" flex items-center gap-2 hover:font-semibold text-xs transition-all duration-75 text-white" to="" >
        ...
        <div className=" right-0 top-0 absolute mr-10 sm:mr-4 lg:flex flex-col hidden items-end  justify-end mt-5 pointer-events-none ">
            ...
            <MenuIcon onClick={()=>{
              setActive(!navActive)
            }} className=" pointer-events-auto  mb-3  sm:h-5 sm:w-5  h-8 w-8 text-white hidden lg:flex cursor-pointer"/>
            :
            <XIcon onClick={()=>{
              setActive(!navActive)
            }} className=" pointer-events-auto mb-3  sm:h-5 sm:w-5   h-8 w-8 text-white hidden lg:flex cursor-pointer"/>
